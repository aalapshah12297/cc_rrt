...
int map_width = (int)(40.0/resolution);
int map_limit = map_width*map_width;
float map_origin_position[3] ={-20.0, -20.0, 0.0};
...
void scanToMap(const sensor_msgs::LaserScan &LaserScan)
{
	...
	for(i = 0; angle_min+angle_increment*i <= angle_max; i++)
	{
		...
		if ((angle_lidar > -55.0*(M_PI/180.0) && angle_lidar < 79*(M_PI/180.0)) ||
...
int main(int argc, char **argv)
{
	...
	ros::Publisher pub_map = n.advertise<nav_msgs::OccupancyGrid>("/map", 1);
